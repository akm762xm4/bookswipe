...
export default function LoginPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // After login, redirect and let server-rendered pages read the cookie
      router.push("/");
    } catch (err: any) {
  ...
  return (
    //align form in center of the page
    <div className="flex items-center justify-center">
      <Card className="w-full max-w-sm p-5">
        <h1 className="text-xl font-semibold mb-3">Welcome back</h1>
        <p className="text-xs text-slate-400 mb-4">Sign in to continue</p>
        <form onSubmit={submit} className="space-y-3">
          ...
          <div className="text-[11px] text-slate-400 text-center">
            No account? <a href="/auth/register" className="text-indigo-400 hover:text-indigo-300">Register</a>
          </div>
        </form>
      </Card>
    </div>
