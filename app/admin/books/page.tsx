...
export default function AdminBooksPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      setSuccess(mode === "edit" ? "Book updated successfully." : "Book created successfully.");
      await loadBooks();
      backToList();
    } catch (e: any) {
      setServerError(e.message);
  ...
  const onDelete = async (book: any) => {
    ...
    setSuccess(null);
    try {
      const res = await fetch(`/api/books/${encodeURIComponent(book.slug)}`, { method: "DELETE" });
      const j = await res.json().catch(() => ({}));
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Button onClick={startCreate}>Create Book</Button>
        </div>
        ...
        <Card className="p-0 overflow-hidden">
          <div className="overflow-x-auto" data-swipe-lock="true">
            ...
            <tbody>
              {books.map((b) => (
                <tr key={b._id} className="border-t border-slate-800 hover:bg-slate-900">
                  ...
                  <td className="px-4 py-2">
                    <div className="flex gap-2">
                      <Button variant="secondary" onClick={() => startEdit(b)}>Edit</Button>
                      <Button variant="secondary" className="bg-red-600 hover:bg-red-500 text-white" onClick={() => onDelete(b)}>Delete</Button>
                    </div>
                  </td>
                </tr>
              ))}
              {books.length === 0 && (
                <tr>
                  <td className="px-4 py-6 text-center text-slate-400" colSpan={6}>No books found.</td>
                </tr>
              )}
            </tbody>
            </table>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-semibold">{mode === "edit" ? "Edit Book" : "Create New Book"}</h1>
        <Button variant="secondary" onClick={backToList}>Back to Listing</Button>
      </div>
      ...
      <Card className="p-4">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Input label="Slug" placeholder="lowercase-with-dashes"
              {...register("slug", {
                required: "Slug is required",
                pattern: { value: /^[a-z0-9-]+$/, message: "Use lowercase letters, numbers and dashes" },
              })}
            />
            {errors.slug && <span className="text-[11px] text-red-400">{errors.slug.message}</span>}

            <Input label="Author" {...register("author", { required: "Author is required" })} />
            {errors.author && <span className="text-[11px] text-red-400">{errors.author.message}</span>}
            ...
            <label className="block space-y-1">
              ...
              />
              {previewUrl && (
                <img src={previewUrl} alt="Cover preview" className="mt-2 h-32 w-auto rounded-xl border border-slate-700" />
              )}
            ...
            />
            {errors.price && <span className="text-[11px] text-red-400">{errors.price.message}</span>}
            ...
            />
            {errors.stock && <span className="text-[11px] text-red-400">{errors.stock.message}</span>}

            <Input label="Tags (comma separated)" placeholder="bestseller,new"
              {...register("tags")}
            />
          </div>
          ...
          <label className="block space-y-1">
            ...
            />
            {errors.description && <span className="text-[11px] text-red-400">{errors.description.message}</span>}
          </label>
          ...
          <div className="flex items-center gap-3">
            <Button type="submit" disabled={isSubmitting}>{isSubmitting ? (mode === "edit" ? "Updating..." : "Creating...") : (mode === "edit" ? "Update Book" : "Create Book")}</Button>
            <Button type="button" variant="secondary" onClick={() => reset({ stock: 0 })}>Reset</Button>
          </div>
        </form>
      </Card>
    </div>
